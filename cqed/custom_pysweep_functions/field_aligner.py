...
class FieldAligner(object):
    ...
    def optimize_x(
        ...
        while wiggle_step >= final_step:
            ...
            self.magnet.x_target(current_pos + direction * wiggle_step)
        ...
        **kwargs,
        ...
        if optimize_first:
            current_objective = self.optimize_x(
                verbose=verbose, return_extra=False, waiting_time=waiting_time, **kwargs
            )
        else:
            current_objective = self.objective()
        ...
        for r in rs:
            ...
            if optimize_strategy == "objective_decrease":
                ...
                if np.any(np.isclose(r, optimize_at, atol=np.diff(rs)[0] / 2)):
