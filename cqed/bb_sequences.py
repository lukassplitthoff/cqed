...
class TriggeredReadoutSequence(BroadBeanSequence):
    ...
    def sequence(self):
        ...
        try:
            ...
            logger.warning('No ro_gate defined in channel map. No readout gate pulse will be generated.')

        try:
            bps['src_gate'] = [(self.pre_trigger_delay() + self.post_trigger_delay(),
                                self.readout_gate_len())]
        except KeyError:
            logger.warning('No src_gate defined in channel map. No src gate pulse will be generated.')
